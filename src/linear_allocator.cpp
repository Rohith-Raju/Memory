...
LinearMemory *LinearMemory::init(std::size_t mem_size, std::size_t allignment) {
  ...
  return new (memory)
      LinearMemory(start, start, start + mem_size, start + mem_size);
}
...
// TODO: Need the nearest multiple of the alignent
//

// TODO: Need the next possoble address based on alignment
unsigned int allignment(unsigned need) {}
